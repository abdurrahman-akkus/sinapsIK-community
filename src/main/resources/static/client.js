...
//Eğer serviseWorker tanımlıysa
if ("serviceWorker" in navigator) {
    try {
        // Subscription var mı?
        checkSubscription();
        // Public key değerini çeker ve serviceWorker'ı aktifleştirir.
        init();
    } catch (e) {
...
async function checkSubscription() {
    ...
    if (subscription) {
        ...
        const response = await fetch("/sinapsIK/isSubscribed", {
            method: 'POST',
...
async function init() {
    fetch('/sinapsIK/publicSigningKey')
            .then(response => new ArrayBuffer(response))
            .then(key => this.publicSigningKey = key)
            .finally(() => console.info('Application Server Public Key fetched from the server'));

    await navigator.serviceWorker.register("/sinapsIK/sw.js", {
        scope: "/sinapsIK/"
    });
...
async function unsubscribe() {
    ...
    if (subscription) {
        ...
        if (successful) {
            ...
            await fetch("/sinapsIK/unsubscribe", {
                method: 'POST',
                body: JSON.stringify({endpoint: subscription.endpoint}),
                headers: {
                    "content-type": "application/json"
...
async function subscribe() {
    ...
    await fetch("/sinapsIK/subscribe", {
        method: 'POST',
